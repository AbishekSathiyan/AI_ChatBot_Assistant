...
const ENV_CONFIG = {
  ...
  PUTER_API_URL: process.env.REACT_APP_PUTER_API_URL || 'https://api.puter.com/v2',
...
const usePuterAI = () => {
  ...
  useEffect(() => {
    ...
    const loadPuterScript = () => {
      return new Promise((resolve, reject) => {
        ...
        script.onerror = (error) => {
          console.error(`❌ Failed to load Puter.ai script (attempt ${retryCount + 1}/${maxRetries}):`, error);
          ...
          if (retryCount < maxRetries) {
            ...
            reject(new Error("Failed to load Puter.ai script after multiple attempts"));
          }
        };
    ...
    loadPuterScript().catch(err => {
      console.warn("⚠️ Using fallback API due to script load failure:", err.message);
      setApiHealth('unhealthy');
    });
    ...
    try {
      ...
      if (!response.ok) {
        ...
        switch (response.status) {
          case 401:
            throw { type: ErrorTypes.AUTH, message: "Invalid API key", status: 401 };
          case 429:
            throw { type: ErrorTypes.RATE_LIMIT, message: "Rate limit exceeded", status: 429 };
          case 503:
          case 504:
            throw { type: ErrorTypes.SERVER, message: "Service unavailable", status: response.status };
          default:
            throw { type: ErrorTypes.UNKNOWN, message: errorData.error || `API Error: ${response.status}`, status: response.status };
        }
      }

      const data = await response.json();
      return data.message?.content || data.response || data.text || "No response generated";
    } catch (error) {
      clearTimeout(timeoutId);
      
      if (error.name === 'AbortError' || error.name === 'TimeoutError') {
        throw { type: ErrorTypes.TIMEOUT, message: "Request timeout", status: 408 };
      }
      
      if (error instanceof TypeError && error.message.includes('fetch')) {
        throw { type: ErrorTypes.NETWORK, message: "Network connection failed", status: 0 };
      }
  ...
  const callPuterAI = useCallback(async (prompt, options = {}) => {
    const {
    ...
    try {
      if (!ENV_CONFIG.USE_DIRECT_API && window.puter?.ai?.chat && isPuterLoaded) {
        try {
          ...
          console.warn("⚠️ SDK failed, falling back to direct API:", sdkError.message);
          return await callDirectAPI(prompt, { model, timeout, temperature, max_tokens });
        }
      }
      ...
      if (retries > 0 && (
        ...
        await new Promise(resolve => setTimeout(resolve, ENV_CONFIG.PUTER_RETRY_DELAY));
      ...
      switch (errorType) {
        ...
        case ErrorTypes.SERVER:
          ...
          break;
        default:
          userFriendlyMessage = "⚠️ An unexpected error occurred. Please try again.";
      }
...
function App() {
  ...
  const { callPuterAI, isLoading: isAPILoading, error: apiCallError, apiHealth } = usePuterAI();
  ...
  const formatMessage = useCallback((text) => {
    ...
    return parts.map((part, index) => {
      if (part.startsWith("```") && part.endsWith("```")) {
        ...
        return (
          <div key={index} className={`code-block-wrapper my-3 md:my-4 group ${!isDarkMode ? 'light' : ''}`}>
            <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-2 ${
              ...
              <div className="flex items-center gap-2">
                <div className={`w-5 h-5 rounded ${isDarkMode ? 'bg-blue-500/10' : 'bg-blue-100'} flex items-center justify-center flex-shrink-0`}>
              ...
              <div className="flex items-center gap-1 ml-auto sm:ml-0">
                ...
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8.25 7.5V6.108c0-1.135.845-2.098 1.976-2.192.373-.03.748-.057 1.123-.08M15.75 18H9a2.25 2.25 0 01-2.25-2.25V9m12 0h.008v.008h-.008V9z" />
                  </svg>
                ...
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M20.25 20.25v-4.5m0 4.5h-4.5m4.5 0L15 15" />
                  </svg>
  ...
  const sendMessage = async () => {
    ...
    if (input.length > ENV_CONFIG.MAX_INPUT_LENGTH) {
      alert(`Input is too long. Maximum length is ${ENV_CONFIG.MAX_INPUT_LENGTH} characters.`);
      return;
    }
  ...
  const readResponse = useCallback((text) => {
    if (!voiceSupported) {
  ...
  const suggestions = useMemo(() => [
    {
      ...
      color: "blue",
      prompt: "Create a React hook for API fetching with loading states and error handling"
    },
    {
      id: 2,
      title: "Python",
  ...
  const renderInputActions = () => (
    <div className="flex items-center gap-0.5 sm:gap-1">
      ...
      <button
        onClick={sendMessage}
        disabled={!input.trim() || isAPILoading || isTyping || isStreaming || apiHealth === 'unhealthy'}
        className={`relative group p-2 sm:p-2.5 md:p-3 rounded-lg sm:rounded-xl transition-all duration-300 ${
          input.trim() && !isAPILoading && !isTyping && !isStreaming && apiHealth !== 'unhealthy'
            ? isDarkMode
      ...
      >
        ...
        {!isMobile && input.trim() && !isAPILoading && apiHealth !== 'unhealthy' && (
          ...
          <div className="max-w-6xl mx-auto flex items-center justify-between relative z-10">
            <div className="flex items-center gap-2 sm:gap-4">
              <div className="relative group">
                <div className={`absolute inset-0 blur-xl sm:blur-2xl opacity-30 animate-pulse rounded-xl sm:rounded-2xl ${
                  isDarkMode ? 'bg-blue-500' : 'bg-blue-400'
                }`}></div>
                <div className={`absolute inset-0 blur-lg sm:blur-xl opacity-20 animate-pulse delay-300 rounded-xl sm:rounded-2xl ${
                ...
                <div className="relative w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-lg sm:rounded-xl md:rounded-2xl overflow-hidden shadow-xl sm:shadow-2xl group-hover:scale-110 transition-transform duration-300">
                  <img 
                    ...
                    onError={(e) => {
                      ...
                      e.target.parentElement.innerHTML += '<span class="text-xl sm:text-2xl md:text-3xl">🤖</span>';
                    }}
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center gap-1 sm:gap-2">
                  ...
                  <span className={`px-1.5 sm:px-2 py-0.5 text-[8px] sm:text-[10px] font-medium rounded-full uppercase tracking-wider ${
                    isDarkMode 
                      ? 'bg-blue-500/10 border border-blue-500/20 text-blue-400' 
                      : 'bg-blue-100 border border-blue-200 text-blue-600'
                  }`}>
                    {ENV_CONFIG.ENVIRONMENT === 'production' ? 'Production' : 'Beta'}
                  </span>
                </div>
                <div className="flex items-center gap-1 sm:gap-2 mt-0.5">
                  <div className="flex items-center gap-1">
                    <span className="relative flex h-1 sm:h-1.5 w-1 sm:w-1.5">
                      <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${
                        apiStatusColor === 'emerald' ? 'bg-emerald-400' : 
                        apiStatusColor === 'red' ? 'bg-red-400' : 'bg-yellow-400'
                      } opacity-75`}></span>
                      <span className={`relative inline-flex rounded-full h-1 sm:h-1.5 w-1 sm:w-1.5 ${
                        apiStatusColor === 'emerald' ? 'bg-emerald-500' : 
                        apiStatusColor === 'red' ? 'bg-red-500' : 'bg-yellow-500'
                      }`}></span>
                    </span>
                  ...
                  {!isMobile && (
                    <>
                      <span className={`text-[8px] sm:text-[10px] ${isDarkMode ? 'text-white/20' : 'text-gray-300'}`}>•</span>
                      <span className={`text-[8px] sm:text-[10px] ${isDarkMode ? 'text-white/40' : 'text-gray-500'}`}>
                        {ENV_CONFIG.ENVIRONMENT === 'production' ? 'Production Ready' : 'Development Mode'}
                      </span>
                    </>
                  )}
                </div>
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <button
                ...
                title="Toggle theme"
              >
                <div className={`absolute top-1 left-1 w-4 sm:w-5 h-4 sm:h-5 rounded-full bg-white shadow-md transform transition-transform duration-500 flex items-center justify-center ${
                  isDarkMode ? 'translate-x-6 sm:translate-x-7' : 'translate-x-0'
                }`}>
                  {isDarkMode ? (
                    <svg className="w-2.5 sm:w-3 h-2.5 sm:h-3 text-indigo-600" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z" />
                    </svg>
                  ) : (
                    <svg className="w-2.5 sm:w-3 h-2.5 sm:h-3 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
        ...
        >
          <div className="max-w-6xl mx-auto px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 space-y-4 sm:space-y-5 md:space-y-6">
            {messages.length === 0 && !isStreaming ? (
              <div className={`min-h-[70vh] sm:min-h-[75vh] md:min-h-[80vh] flex flex-col items-center justify-center text-center space-y-6 sm:space-y-8 md:space-y-10 ${
                isDarkMode ? 'text-white' : 'text-gray-900'
              }`}>
                {/* Welcome content */}
                <div className="relative group">
                  <div className={`absolute inset-0 blur-[50px] sm:blur-[80px] md:blur-[100px] opacity-30 rounded-full animate-pulse ${
                    isDarkMode ? 'bg-blue-500' : 'bg-blue-400'
                  }`}></div>
                  <div className={`absolute inset-0 blur-[40px] sm:blur-[60px] md:blur-[80px] opacity-20 rounded-full animate-pulse delay-300 ${
                    isDarkMode ? 'bg-indigo-500' : 'bg-indigo-400'
                  }`}></div>
                  
                  <div className={`relative w-20 h-20 sm:w-24 sm:h-24 md:w-32 md:h-32 rounded-2xl sm:rounded-2xl md:rounded-3xl backdrop-blur-2xl border-2 flex items-center justify-center group-hover:scale-110 transition-all duration-500 overflow-hidden ${
                    isDarkMode 
                ...
                <div className="space-y-2 sm:space-y-3 md:space-y-4 max-w-2xl px-4">
                  <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-light tracking-tight">
                    ...
                    <span className="bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 bg-clip-text text-transparent animate-gradient relative">
                      help
                      <span className={`absolute -bottom-1 sm:-bottom-2 left-0 w-full h-0.5 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full ${
                        isDarkMode ? 'opacity-100' : 'opacity-70'
                      }`}></span>
                    </span>
                    {' '}you today?
                  </h2>
                  <p className={`text-xs sm:text-sm font-light max-w-lg mx-auto leading-relaxed px-2 ${
                ...
                <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full ${
                  ...
                  <span className={`relative flex h-2 w-2`}>
                    <span className={`animate-ping absolute inline-flex h-full w-full rounded-full bg-${apiStatusColor}-400 opacity-75`}></span>
                    <span className={`relative inline-flex rounded-full h-2 w-2 bg-${apiStatusColor}-500`}></span>
                  </span>
                ...
                <div className={`flex flex-wrap justify-center items-center gap-3 sm:gap-4 md:gap-6 text-[10px] sm:text-xs ${
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3 w-full max-w-4xl mt-4 sm:mt-6 md:mt-8 px-2 sm:px-4">
                  {suggestions.slice(0, isMobile ? 6 : 9).map((suggestion) => (
                    ...
                    >
                      <div className={`absolute inset-0 bg-gradient-to-r from-${suggestion.color}-500/0 via-${suggestion.color}-500/0 to-${suggestion.color}-500/0 group-hover:from-${suggestion.color}-500/5 group-hover:via-${suggestion.color}-500/5 group-hover:to-${suggestion.color}-500/0 transition-all duration-700`}></div>
                      
                      <div className="relative z-10">
                        <div className={`w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-lg flex items-center justify-center mb-1.5 sm:mb-2 md:mb-3 group-hover:scale-110 transition-transform duration-300 mx-auto sm:mx-0 ${
                          isDarkMode
                        ...
                        <h3 className={`text-xs sm:text-sm font-medium transition-colors duration-300 ${
                          ...
                          {isMobile ? suggestion.title.substring(0, 4) : suggestion.title}
                        </h3>
                        {!isMobile && (
                          <p className={`text-[8px] sm:text-[10px] mt-1 transition-colors duration-300 ${
                            isDarkMode
              ...
              <>
                {messages.map((msg, idx) => (
                  ...
                  >
                    ...
                    >
                      {msg.sender === "ai" && (
                        <>
                          <div className={`absolute -left-2 sm:-left-3 top-1/2 -translate-y-1/2 w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 flex items-center justify-center shadow-lg ${
                      ...
                      <div className="px-3 sm:px-4 md:px-5 py-2.5 sm:py-3 md:py-4">
                        ...
                        <div className={`flex items-center justify-between mt-2 sm:mt-3 pt-1.5 sm:pt-2 border-t ${
                          msg.sender === "user" 
                            ? isDarkMode ? 'border-blue-400/20' : 'border-white/20'
                            : isDarkMode ? 'border-white/[0.05]' : 'border-gray-200'
                        }`}>
                          <div className={`text-[8px] sm:text-[9px] tracking-wider uppercase flex items-center gap-1 ${
                            msg.sender === "user" 
                              ? 'text-blue-200/50' 
                              : isDarkMode ? 'text-white/20' : 'text-gray-400'
                          }`}>
                          ...
                          {msg.sender === "ai" && extractCode(msg.text) && (
                            <div className="flex items-center gap-2">
                              <span className={`text-[8px] sm:text-[9px] flex items-center gap-1 ${
                ...
                {isStreaming && (
                  <div className="flex justify-start animate-slideIn px-1 sm:px-0">
                    <div className={`relative max-w-[90%] sm:max-w-[80%] md:max-w-[70%] backdrop-blur-sm border rounded-2xl rounded-tl-none ${
                      isDarkMode
                      ...
                      <div className="px-3 sm:px-4 md:px-5 py-2.5 sm:py-3 md:py-4">
                        ...
                        <div className={`flex flex-wrap items-center gap-2 sm:gap-3 mt-2 sm:mt-3 pt-1.5 sm:pt-2 border-t ${
                          isDarkMode ? 'border-white/[0.05]' : 'border-gray-200'
                        }`}>
                          <div className="flex items-center gap-2">
                            <div className="flex gap-1">
                              <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 bg-blue-400 rounded-full animate-bounce" style={{ animationDelay: "0ms" }}></span>
                              <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 bg-indigo-400 rounded-full animate-bounce" style={{ animationDelay: "150ms" }}></span>
                              <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 bg-purple-400 rounded-full animate-bounce" style={{ animationDelay: "300ms" }}></span>
                            </div>
                            <span className={`text-[8px] sm:text-[10px] uppercase tracking-wider ${
                              isDarkMode ? 'text-white/40' : 'text-gray-500'
                            }`}>
                              {isMobile ? 'AI typing...' : 'Generating response'}
                            </span>
                          </div>
                          <span className={`text-[8px] sm:text-[9px] flex items-center gap-1 ${
                            isDarkMode ? 'text-white/20' : 'text-gray-400'
                          }`}>
                            <svg className="w-2 h-2 sm:w-2.5 sm:h-2.5 animate-spin-slow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {isTyping && !isStreaming && (
                  <div className="flex justify-start animate-slideIn px-1 sm:px-0">
                    <div className={`relative backdrop-blur-sm border rounded-2xl rounded-tl-none px-4 sm:px-6 py-3 sm:py-4 ${
                      isDarkMode
                      ...
                      <div className="flex items-center gap-2 sm:gap-4 pl-2 sm:pl-2">
                        <div className="flex gap-1 sm:gap-1.5">
                          <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-blue-400 rounded-full animate-pulse" style={{ animationDelay: "0ms" }}></span>
                          <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-indigo-400 rounded-full animate-pulse" style={{ animationDelay: "200ms" }}></span>
                          <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-purple-400 rounded-full animate-pulse" style={{ animationDelay: "400ms" }}></span>
                        </div>
        ...
        <div className={`relative border-t transition-colors duration-300 ${
          ...
          <div className="max-w-6xl mx-auto px-3 sm:px-4 py-2 sm:py-3 md:py-4 relative z-10">
            <div className="relative">
              <div className={`relative flex items-end gap-2 sm:gap-3 rounded-xl sm:rounded-2xl border transition-all duration-300 px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 ${
                isDarkMode
                ...
                <textarea
                  ...
                  onKeyDown={handleKey}
                  placeholder={isMobile ? "Ask anything..." : "Ask me anything — code, explain, debug, optimize..."}
              ...
              <div className="flex flex-wrap justify-between items-center mt-1.5 sm:mt-2 md:mt-2.5 px-1 sm:px-2">
                <div className="flex items-center gap-2 sm:gap-4">
                  <span className={`text-[8px] sm:text-[9px] md:text-[10px] flex items-center gap-1 sm:gap-1.5 ${
                    isDarkMode ? 'text-white/30' : 'text-gray-500'
                  }`}>
                    <svg className="w-2 h-2 sm:w-2.5 sm:h-2.5 md:w-3 md:h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 9.75v6.75m3-6.75v6.75m3-6.75v6.75M9 9.75h7.5M9 12h7.5M9 15h7.5M3 18h18M3 6h18" />
                    </svg>
                    {!isMobile && <span>⏎ Send · </span>}
                    <span className={isDarkMode ? 'text-white/50' : 'text-gray-600'}>
                      {isMobile ? '↵' : '⇧+⏎ New line'}
                    </span>
                  </span>
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                  <span className={`text-[8px] sm:text-[9px] md:text-[10px] font-mono flex items-center gap-1 sm:gap-1.5 ${
                    isDarkMode ? 'text-white/30' : 'text-gray-500'
                  }`}>
                    <svg className="w-2 h-2 sm:w-2.5 sm:h-2.5 md:w-3 md:h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M20.25 20.25v-4.5m0 4.5h-4.5m4.5 0L15 15" />
                    </svg>
                    <span className={input.length > ENV_CONFIG.MAX_INPUT_LENGTH * 0.875 
                      ? isDarkMode ? 'text-yellow-500/70' : 'text-yellow-600' 
                      : isDarkMode ? 'text-white/30' : 'text-gray-500'
                    }>
                      {isMobile ? `${input.length}/${ENV_CONFIG.MAX_INPUT_LENGTH}` : `${input.length}/${ENV_CONFIG.MAX_INPUT_LENGTH}`}
                    </span>
                  </span>
                </div>
              </div>
        ...
        {showCodeModal && (
          <div className="fixed inset-0 bg-black/90 backdrop-blur-2xl z-50 flex items-center justify-center p-2 sm:p-3 md:p-4 animate-fadeIn">
            <div className={`${
              ...
              <div className={`flex flex-col sm:flex-row sm:items-center justify-between px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 md:py-4 border-b gap-2 sm:gap-0 ${
                isDarkMode ? 'border-white/[0.08] bg-gradient-to-r from-white/[0.02] to-transparent' : 'border-gray-200 bg-gradient-to-r from-gray-50 to-transparent'
              }`}>
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 rounded-lg sm:rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-blue-500/30 overflow-hidden flex-shrink-0">
                    <img 
                      ...
                      onError={(e) => {
                        e.target.style.display = 'none';
                        e.target.parentElement.innerHTML += '<span class="text-white text-sm">🤖</span>';
                      }}
                    />
                  </div>
                  <div>
                    <div className="flex items-center gap-1 sm:gap-2">
                      <h3 className={`text-sm sm:text-base font-medium ${isDarkMode ? 'text-white/90' : 'text-gray-900'}`}>
                        {isMobile ? 'Code' : 'Code Preview'}
                      </h3>
                      <span className={`px-1.5 sm:px-2 py-0.5 text-[8px] sm:text-[9px] font-medium rounded-full uppercase tracking-wider ${
                        isDarkMode
                          ? 'bg-blue-500/10 border border-blue-500/20 text-blue-400'
                          : 'bg-blue-100 border border-blue-200 text-blue-600'
                      }`}>
                    ...
                    {!isMobile && (
                      <p className={`text-[9px] sm:text-[10px] mt-0.5 ${isDarkMode ? 'text-white/40' : 'text-gray-500'}`}>
                ...
                <div className="flex items-center gap-1 sm:gap-2 self-end sm:self-auto">
                  ...
                  >
                    <svg className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${
                      isDarkMode ? 'text-white/60 group-hover:text-white/80' : 'text-gray-600 group-hover:text-gray-800'
                    ...
                    <span className={`text-xs sm:text-xs font-medium ${
                      ...
                      {copySuccess ? 'Copied!' : isMobile ? 'Copy' : 'Copy Code'}
                    </span>
                  </button>
                  <button
                    ...
                    title="Close modal"
                  >
                    <svg className={`w-4 h-4 sm:w-5 sm:h-5 ${isDarkMode ? 'text-white/60' : 'text-gray-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M6 18L18 6M6 6l12 12" />
                    </svg>
                  </button>
                </div>
              ...
              <div className={`flex-1 overflow-y-auto p-3 sm:p-4 md:p-6 ${isDarkMode ? 'bg-[#0A0A0A]/50' : 'bg-gray-50/50'}`}>
                <pre className={`rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6 overflow-x-auto border text-xs sm:text-sm ${
                  isDarkMode
                    ? 'bg-gradient-to-b from-[#1E1E1E] to-[#1A1A1A] border-white/[0.05]'
                    : 'bg-gradient-to-b from-white to-gray-50 border-gray-200'
                }`}>
                  <code ref={codeRef} className={`language-${codeLanguage} font-mono whitespace-pre-wrap leading-relaxed ${
